...
class RTCMReader:
    ...
    def _read_bytes(self, size: int) -> bytes:
        ...
        if 0 < len(data) < size:  # truncated stream
            # Check if this is a BytesIO stream (memory buffer)
            # For BytesIO, reaching the end is normal, not an error
            if isinstance(self._stream, io.BytesIO):
                raise EOFError()  # Treat as normal EOF for memory streams
            else:
                # For real serial/socket streams, this is an unexpected termination
                stream_type = "Serial" if hasattr(self._stream, 'port') else "Stream"
                raise RTCMStreamError(
                    f"{stream_type} stream terminated unexpectedly. "
